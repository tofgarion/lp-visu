...
to put cuts for Integer Linear Programming.
...
- ~lp_visu_ex.py~ which is a simple example for ILP in which I add
...
~lp_visu_notebook.py~ contains a simpler version of LPVisu designed to
be used with notebooks due to problems with
matplotlib. ~lp_visu_ex.ipynb~ is a notebook example.
